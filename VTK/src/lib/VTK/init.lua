...
local Panel = require("VTK/core/Panel")
...
vtk.init = function()
	...
	for _, func in pairs(api_functions) do
		frame[func] = function(...)
	...
	if not frame.setBackground then
		error("no tiene setBackground")
	end

	_ENV.Redraw = function()
		frame:redraw_handler()
		frame.setBackground(frame.background_color)
	end
	_ENV.Touch = function(...)
		frame:touch_handler(...)
		frame.setBackground(frame.background_color)
